...
public class MainFormPresenterTest {
	...
	public void TestMainForm1() {
		...
		// We'll use these to capture values passed from the Presenter
		// to the other objects

		Capture<MainFormPresenter> mfpCapture= new Capture<MainFormPresenter>();
		Capture<AsyncCallback<Integer>> acbCapture= new Capture<AsyncCallback<Integer>>();
		...
		replay(environmentMock);
		replay(formViewMock);

		// ...and then go do the job!

		MainFormPresenter formPresenter= new MainFormPresenter(environmentMock,
		        formViewMock);
		formPresenter.onAverageClick();
		acbCapture.getValue().onSuccess(5);
